...
to setup_world
  ...
  clear-turtles
  ask patches [
    if pxcor < 0 [set pcolor pink]
    if pxcor >= 0 [set pcolor gray]
  ]

  set student_id "23071602" ;
  set student_name "Aryan Patil" ;
...
to setup_agents
  ...
  create-turtles pink_population [
    ...
    set antibodies 0
    setxy random-float -45 random-ycor
  ]
...
to spread_infection
    ...
    if any? other turtles in-radius 1 with [color = green] [
      if random 100 < infection_rate and antibodies = 0 and color != green [
        set infected_time illness_duration
        set color green
      ]
    ]
end
to enforce_travel_restrictions
...
to run_model
  ...
  ask turtles [
    ; Move turtles
    if not self_isolation [fd 0.3]
    set heading heading + random 20 - random 20

     if travel_restrictions [ enforce_travel_restrictions ]
    if social_distancing [ enforce_social_distancing ]

    spread_infection
    if infected_time > 0 [
      set infected_time infected_time - 1
      if infected_time = undetected_period [
        if self_isolation  [

          set color blue ]
        if self_isolation and color = blue [fd 0]
      ]
      if infected_time = 0 [
  ifelse random 100 < survival_rate [
    ...
    if antibodies > 0 [
      ...
      if antibodies = 0 [set color yellow]
    ]
  ]
  update_statistics
end
...
to update_statistics
  set total_population pink_population + gray_population
  ...
  set pink_antibodies_percentage (count turtles with [antibodies > 0 and group = "pink turtle"] / count turtles with [group = "pink turtle"]) * 100
  set gray_antibodies_percentage (count turtles with [antibodies > 0 and group = "gray turtle"] / count turtles with [group = "gray turtle"]) * 100
  ]
  set active_cases count turtles with [color = green]
end
...
"" ""
PENS
"pink population" 1.0 0 -2064490 true "" "plot current_pink_population"
"gray population" 1.0 0 -7500403 true "" "plot current_gray_population"
